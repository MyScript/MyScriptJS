...
function runInkTests(ink) {
  ...
  //   inkPlayer.checkConvert(browser, config, ink.strokes, ink.exports.LATEX, '#result span', '#result');
  // };

  module.exports[config.header + ' checkUndoRedoReconnect ' + ink.name] = function checkUndoRedoReconnect(browser) {
    inkPlayer.checkUndoRedoReconnect(browser, config, ink.strokes, ink.exports.LATEX, '#result span', '#result');
  };
}
