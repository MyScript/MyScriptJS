...
export function computeStats(model) {
  ...
  if (model.pendingStrokes || model.recognizedStrokes) {
    const pendingStrokes = Object.keys(model.pendingStrokes).filter(key => model.pendingStrokes[key] !== undefined).map(key => model.pendingStrokes[key]).reduce((a, b) => a.concat(b), []);
